...
async function bootstrap() {
  ...
  const configuredOrigins = (config.get<string>('FRONTEND_URL') ?? 'http://localhost:3000')
    .split(',')
  ...
  // ---- CORS ----
  const originValidator: NonNullable<CorsOptions['origin']> = (origin, callback) => {
  ...
  try {
    app.useGlobalPipes(
      new ValidationPipe({
        ...
        transformOptions: canUseTransform ? { enableImplicitConversion: true } : undefined,
      }),
    )
  } catch {
  ...
  app.getHttpAdapter().get('/health', (_req: unknown, res: { json: (d: unknown) => void }) => {

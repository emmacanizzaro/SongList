...
export class MeetingsService {
  ...
  async update(churchId: string, meetingId: string, dto: Partial<CreateMeetingDto>) {
  ...
  async addSong(
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        throw new ConflictException('Esa canción ya está agregada a la reunión')
      }
      throw error
    }
  }

  async reorderSongs(churchId: string, meetingId: string, orderedSongIds: string[]) {
    await this.assertBelongsToChurch(meetingId, churchId)
  ...
  async unassignMusician(churchId: string, meetingId: string, assignmentId: string) {
    await this.assertBelongsToChurch(meetingId, churchId)

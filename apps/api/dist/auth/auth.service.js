...
let AuthService = AuthService_1 = class AuthService {
    ...
    async register(dto) {
        ...
        if (dto.inviteToken) {
            ...
            if (existingInvitedUser) {
                ...
                const isPendingInvite = existingInvitedUser.name.startsWith('Invitado ') && Boolean(existingMembership);
                if (!isPendingInvite) {
        ...
        if (existing) {
            const isPendingInvite = existing.name.startsWith('Invitado ') && existing.memberships.length > 0;
            if (!isPendingInvite) {
                throw new common_1.ConflictException('El email ya está registrado');
            }
            const passwordHash = await bcrypt.hash(dto.password, 12);
    ...
    async logout(rawRefreshToken) {
        const tokenHash = this.hashToken(rawRefreshToken);
        await this.prisma.refreshToken.delete({ where: { tokenHash } }).catch(() => null);
    }

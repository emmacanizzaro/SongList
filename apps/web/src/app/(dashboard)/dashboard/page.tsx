...
export default function DashboardPage() {
  ...
  const hasMembersLimitReached = Boolean(
    membersQuota && !membersQuota.unlimited && (membersQuota.remaining ?? 0) <= 0,
  )
    ...
    <>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        <section
          ...
          className="relative overflow-hidden rounded-[32px] bg-slate-950 px-6 py-8 text-white shadow-[0_28px_60px_rgba(15,23,42,0.22)] sm:px-8 lg:px-10"
        >
          ...
          <div className="relative flex flex-col gap-8">
            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
              <div className="max-w-2xl">
                <span className="eyebrow bg-white/10 text-slate-200">Panel principal</span>
                <h1 className="mt-5 text-3xl font-semibold tracking-tight sm:text-4xl">
                  Bienvenido, {user?.name?.split(' ')[0]}.
                </h1>
                <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300 sm:text-base">
                  Visualiza próximas reuniones, biblioteca musical y estado general del equipo desde
                  un tablero más claro, centrado y preparado para trabajar rápido.
                </p>
              </div>

              <div className="flex flex-wrap gap-3 xl:justify-end">
                <Link
                  data-tour="dashboard-new-song"
                ...
                <Link
                  data-tour="dashboard-new-meeting"
                  href="/meetings/new"
            ...
            <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-1">
                ...
                />
                <HeroMetric label="Biblioteca" value={`${stats?.songsCount ?? 0} canciones`} />
                <HeroMetric label="Equipo" value={`${stats?.membersCount ?? 0} miembros`} />
              </div>
            </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-semibold text-slate-900 dark:text-white">Próximas reuniones</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">

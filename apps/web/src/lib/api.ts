...
export const songsApi = {
  ...
  addVersion: (id: string, data: object) => api.post(`/api/v1/songs/${id}/versions`, data),
}

export const meetingsApi = {
  list: (upcoming?: boolean) => api.get('/api/v1/meetings', { params: { upcoming } }),
  get: (id: string) => api.get(`/api/v1/meetings/${id}`),
  create: (data: object) => api.post('/api/v1/meetings', data),
  update: (id: string, data: object) => api.patch(`/api/v1/meetings/${id}`, data),
  ...
  generateShare: (meetingId: string) => api.post(`/api/v1/meetings/${meetingId}/share`),
  assign: (meetingId: string, data: { userId: string; instrumentId: string; notes?: string }) =>
...
export const churchApi = {
  ...
  invite: (email: string, role: string) => api.post('/api/v1/churches/me/members', { email, role }),
  createInviteLink: (email: string, role: string) =>
    api.post('/api/v1/churches/me/invites', { email, role }),
  getInvitePublic: (token: string) => api.get(`/api/v1/churches/invites/${token}/public`),
  updateMemberRole: (memberId: string, role: string) =>
    api.patch(`/api/v1/churches/me/members/${memberId}/role`, { role }),
  removeMember: (memberId: string) => api.delete(`/api/v1/churches/me/members/${memberId}`),
}

export const subscriptionsApi = {
  get: () => api.get('/api/v1/subscriptions'),
  entitlements: () => api.get('/api/v1/subscriptions/entitlements'),
  checkout: (plan: string) => api.post('/api/v1/subscriptions/checkout', { plan }),
